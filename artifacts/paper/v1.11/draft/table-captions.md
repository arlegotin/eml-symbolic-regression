...
`artifacts/paper/v1.11/draft/tables/motif-library-evolution.md` records before/after depth and node counts for reusable compiler motifs. Planck uses the v1.11 diagnostic convention 24 -> 14; older relaxed-depth references are noted separately.

## Table 3. Logistic and Planck negative results

`artifacts/paper/v1.11/draft/tables/logistic-planck-negative-results.md` makes the unsupported status visible: compile support is unsupported, relaxed depth improved, blind probes did not recover, and promotion remains `no`.
