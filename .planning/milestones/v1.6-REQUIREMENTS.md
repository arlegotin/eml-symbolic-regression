# Requirements Archive: v1.6 Hybrid Search Pipeline and Exact Candidate Recovery
...
- [x] **REFI-01**: User can freeze a snapped exact tree, expose its literal constants, and optimize those coefficients post-snap without changing the discrete structure.
- [x] **REFI-02**: User receives both pre-refit and post-refit exact-candidate artifacts, and the refit candidate is accepted only when it improves or matches the fallback under verifier-owned ranking.

### Numerical Stability and Domain Control

- [x] **STAB-01**: User can inspect training diagnostics for `exp` overflow, `log`-domain violations, non-finite intermediates, clamp counts, and other branch or domain anomalies rather than only `exp` clamping.
- [x] **STAB-02**: User can enable positive-domain-safe parameterizations or penalties for log-feeding branches during training without changing faithful verification semantics after snapping.

### Compiler Macro Shortening and Warm Starts

- [x] **COMP-01**: User can compile supported small primitives and canonical motifs through a validated short-macro library before falling back to the existing generic compiler rules.
- [x] **COMP-02**: User can inspect compiler diagnostics that report macro hits, missed shortcut opportunities, and depth/node deltas against the old compiler output.
- [x] **COMP-03**: User gets expanded warm-start coverage for currently depth-limited formulas while preserving the existing fail-closed compiler behavior when no validated short macro applies.

### Evaluation Integrity and Regression Evidence

- [x] **EVAL-01**: User can run proof and campaign suites that keep pure blind, scaffolded blind, compile-only, warm-start, and perturbed-basin modes explicitly separate in run artifacts and aggregate reports.
- [x] **EVAL-02**: User can compare v1.6 recovery results against archived v1.5 proof artifacts and archived v1.4 campaign baselines without overwriting those prior milestone outputs.
- [x] **EVAL-03**: User can run regression tests that fail if the new candidate-pool, cleanup, refit, or macro stages regress below the fallback exact-candidate performance on declared benchmark cases.
- [x] **EVAL-04**: User receives reports and docs that state weak-dominance claims separately from likely-improvement claims and do not overstate blind-discovery capability.
...
*Last updated: 2026-04-16 after completing Phase 38 evaluation and regression lockdown*
